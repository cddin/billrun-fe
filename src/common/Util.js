  ...
  if (!configCache.has(path[0])) {
    switch (path[0]) {
      case "env":
        ...
        break;
      case "reports":
        configCache = configCache.set(
          "reports",
          Immutable.fromJS(reportConfig)
        );
        break;
      case "fieldNames":
        configCache = configCache.set(
          "fieldNames",
          Immutable.fromJS(fieldNamesConfig)
        );
        break;
      case "systemItems":
        configCache = configCache.set(
          "systemItems",
          Immutable.fromJS(systemItemsConfig)
        );
        break;
      case "mainMenu":
        configCache = configCache.set("mainMenu", Immutable.fromJS(mainMenu));
        break;
      case "events":
        configCache = configCache.set("events", Immutable.fromJS(eventsConfig));
        break;
      case "rates":
        configCache = configCache.set("rates", Immutable.fromJS(ratesConfig));
        break;
      case "import":
        configCache = configCache.set("import", Immutable.fromJS(importConfig));
        break;
      case "export":
        configCache = configCache.set("export", Immutable.fromJS(exportConfig));
        break;
      case "collections":
        configCache = configCache.set(
          "collections",
          Immutable.fromJS(collectionsConfig)
        );
        break;
      case "customFields":
        configCache = configCache.set(
          "customFields",
          Immutable.fromJS(customFieldsConfig)
        );
        break;
      case "inputProcessor":
        configCache = configCache.set(
          "inputProcessor",
          Immutable.fromJS(inputProcessorConfig)
        );
        break;
      case "discount":
        configCache = configCache.set(
          "discount",
          Immutable.fromJS(discountConfig)
        );
        break;
      default:
        console.log(`Config category not exists ${path}`);
    }
  }
...
export const buildPageTitle = (mode, entityName, item = Immutable.Map()) => {
  switch (mode) {
    ...
    case "create": {
      ...
      if (entitySettings) {
        return `Create New ${titleCase(
          entitySettings.get("itemName", entitySettings.get("itemType", ""))
        )}`;
      }
      return "Create";
    }
    ...
    case "closeandnew": {
      ...
      if (entitySettings) {
        if (entityName === "customer") {
          return `Edit ${titleCase(
            entitySettings.get("itemName", entitySettings.get("itemType", ""))
          )} - ${getFirstName(item)} ${getLastName(item)} [${getCustomerId(
            item
          )}]`;
        }
        if (entityName === "subscription") {
          return `Edit ${titleCase(
            entitySettings.get("itemName", entitySettings.get("itemType", ""))
          )} - ${getFirstName(item)} ${getLastName(item)} [${getSubscriberId(
            item
          )}]`;
        }
        ...
        return `Edit ${titleCase(
          entitySettings.get("itemName", entitySettings.get("itemType", ""))
        )} - ${item.get(entitySettings.get("uniqueField", ""), "")}`;
      }
      return "Edit";
    }
...
export const isItemClosed = (item) => {
  const earlyExpiration = item.getIn(
    ["revision_info", "early_expiration"],
    null
  );
  ...
  if (minDate && getItemId(item, false)) {
    return moment.max(
      ...
      getItemDateValue(
        item,
        "originalValue",
        getItemDateValue(item, "from", moment(0))
      )
    );
  }
  // only item
  if (getItemId(item, false)) {
    return getItemDateValue(
      item,
      "originalValue",
      getItemDateValue(item, "from", moment(0))
    );
...
export const getRevisionStartIndex = (item, revisions) => {
  const index = revisions.findIndex(
    (revision) => getItemId(revision) === getItemId(item)
  );
  if (index <= 0) {
    return 0;
  }
...
export const parseConfigSelectOptions = (configOption) =>
  formatSelectOptions(
    configOption.has("title")
      ? Immutable.Map({
          value: configOption.get("id"),
          label: configOption.get("title"),
        })
      : configOption.get("id")
  );

export const parseFieldSelectOptions = (fieldOption) =>
  formatSelectOptions(
    Immutable.Map({
...
export const isUpdaterField = (field = Immutable.Map()) =>
  field.get("unique", false) ||
  (field.get("field_name", "") === "key" &&
    field.get("mandatory", false) &&
    field.get("system", false)) ||
  (field.get("field_name", "") === "name" &&
    field.get("mandatory", false) &&
    field.get("system", false));

export const createReportColumnLabel = (
  label,
  fieldsOptions,
  opOptions,
  oldField,
  ...
  const oldFieldLabel =
    oldField === ""
      ...
      : fieldsOptions
          .find(
            (fieldConfig) => fieldConfig.get("id") === oldField,
            null,
            Immutable.Map({ title: newField })
          )
          .get("title", "");
  const newFieldLabel =
    oldField === newField
      ...
      : fieldsOptions
          .find(
            (fieldConfig) => fieldConfig.get("id") === newField,
            null,
            Immutable.Map({ title: newField })
          )
          .get("title", "");
  ...
  const oldOpLabel =
    oldOp === ""
      ...
      : opOptions
          .find(
            (groupByOperator) => groupByOperator.get("id") === oldOp,
            null,
            Immutable.Map()
          )
          .get("title", "");
  const newOpLabel =
    oldOp === newOp
      ...
      : opOptions
          .find(
            (groupByOperator) => groupByOperator.get("id") === newOp,
            null,
            Immutable.Map()
          )
          .get("title", "");
  ...
  const oldLabel =
    oldOpLabel === "" || oldOp === "group"
      ? oldFieldLabel
      : `${oldFieldLabel} (${oldOpLabel})`;
  if (label === "" || label === oldLabel) {
    return newOpLabel === "" || newOp === "group"
      ? newFieldLabel
      : `${newFieldLabel} (${newOpLabel})`;
  }
  return label;
};
...
export const getSettingsKey = (entityName, asArray = false) => {
  const key = getConfig(
    ["systemItems", getFieldEntityKey(entityName), "settingsKey"],
    entityName
  );
  if (asArray) {
    return key.split(".");
...
export const getRateByKey = (rates, rateKey) =>
  rates.find((rate) => rate.get("key", "") === rateKey) || Immutable.Map();

export const getRateUsaget = (rate) =>
  rate.get("rates", Immutable.Map()).keySeq().first() || "";

export const getRateUsagetByKey = (rates, rateKey) =>
  getRateUsaget(getRateByKey(rates, rateKey));

export const getRateUnit = (rate, usaget) =>
  rate.getIn(["rates", usaget, "BASE", "rate", 0, "uom_display", "range"], "");

export const getUom = (propertyTypes, usageTypes, usaget) => {
  const selectedUsaget =
    usageTypes.find(
      (usageType) => usageType.get("usage_type", "") === usaget
    ) || Immutable.Map();
  return (
    propertyTypes.find(
      (prop) => prop.get("type", "") === selectedUsaget.get("property_type", "")
    ) || Immutable.Map()
...
export const getUsagePropertyType = (usageTypesData, usage) =>
  (
    usageTypesData.find((usaget) => usaget.get("usage_type", "") === usage) ||
    Immutable.Map()
  ).get("property_type", "");
...
export const getUnitLabel = (propertyTypes, usageTypes, usaget, unit) => {
  ...
  return (
    uom.find((propertyType) => propertyType.get("name", "") === unit) ||
    Immutable.Map()
  ).get("label", "");
};
...
export const getValueByUnit = (
  ...
  return value
    .toString()
    .split(",")
    .map((val) => (toBaseUnit ? val * u : val / u))
    .join();
};

const getItemConvertedRates = (
  propertyTypes,
  usageTypes,
  item,
  toBaseUnit,
  ...
  const convertedRates = item
    .get("rates", Immutable.Map())
    .withMutations((ratesWithMutations) => {
      ratesWithMutations.forEach((rates, usagetOrPlan) => {
        rates.forEach((rate, planOrUsaget) => {
          ...
          rate.get("rate", Immutable.List()).forEach((rateStep, index) => {
            const rangeUnit = rateStep.getIn(
              ["uom_display", "range"],
              "counter"
            );
            const intervalUnit = rateStep.getIn(
              ["uom_display", "interval"],
              "counter"
            );
            const convertedFrom = getValueByUnit(
              propertyTypes,
              usageTypes,
              usaget,
              rangeUnit,
            ...
            const newFrom = isNumber(convertedFrom)
              ? parseFloat(convertedFrom)
              : convertedFrom;
            ...
            const newTo = isNumber(convertedTo)
              ? parseFloat(convertedTo)
              : convertedTo;
            const price = rateStep.get("price");
            const convertedPrice = isNumber(price) ? parseFloat(price) : price;
            const convertedInterval = getValueByUnit(
              propertyTypes,
              usageTypes,
              usaget,
              intervalUnit,
              rateStep.get("interval"),
              toBaseUnit
            );
            const newInterval = isNumber(convertedInterval)
              ? parseFloat(convertedInterval)
              : convertedInterval;
            const ratePath =
              type === "product"
                ? [usaget, plan, "rate", index]
                : [plan, usaget, "rate", index];
          ...
          if (percentage !== null) {
            const ratePath =
              type === "product" ? [usaget, plan] : [plan, usaget];
            const convertedPercentage = toBaseUnit
              ? percentage / 100
              : percentage * 100;
            ratesWithMutations.setIn(
              [...ratePath, "percentage"],
              convertedPercentage
            );
          }
  ...
  return !convertedRates.isEmpty() ? convertedRates : Immutable.Map();
};

export const getProductConvertedRates = (
  propertyTypes,
  usageTypes,
  item,
  toBaseUnit = true
) =>
  getItemConvertedRates(propertyTypes, usageTypes, item, toBaseUnit, "product");

export const getPlanConvertedRates = (
  propertyTypes,
  usageTypes,
  item,
  toBaseUnit = true
) => getItemConvertedRates(propertyTypes, usageTypes, item, toBaseUnit, "plan");

export const getPlanConvertedPpThresholds = (
  propertyTypes,
  usageTypes,
  ppIncludes,
  item,
  ...
  // eslint-disable-line max-len
  const convertedPpThresholds = item
    .get("pp_threshold", Immutable.Map())
    .withMutations((ppThresholdsWithMutations) => {
      ppThresholdsWithMutations.forEach((value, ppId) => {
        const ppInclude =
          ppIncludes.find(
            (pp) => pp.get("external_id", "") === parseInt(ppId)
          ) || Immutable.Map();
        const unit = ppInclude.get("charging_by_usaget_unit", false);
        if (unit) {
          const usaget = ppInclude.get("charging_by_usaget", "");
          const newValue = getValueByUnit(
            propertyTypes,
            usageTypes,
            usaget,
            unit,
...
export const getPlanConvertedNotificationThresholds = (
  propertyTypes,
  usageTypes,
  ppIncludes,
  item,
  ...
  // eslint-disable-line max-len
  const convertedPpThresholds = item
    .get("notifications_threshold", Immutable.Map())
    .withMutations((notificationThresholdsWithMutations) => {
      notificationThresholdsWithMutations.forEach((notifications, ppId) => {
        ...
        if (unit) {
          ...
          notifications.forEach((notification, index) => {
            ...
            notificationThresholdsWithMutations.setIn(
              [ppId, index, "value"],
              parseFloat(newValue)
            );
          });
...
export const getPlanConvertedPpIncludes = (
  propertyTypes,
  usageTypes,
  ppIncludes,
  item,
  ...
  // eslint-disable-line max-len
  const convertedIncludes = item
    .get("include", Immutable.Map())
    .withMutations((includesWithMutations) => {
  ...
  return !convertedIncludes.isEmpty() ? convertedIncludes : Immutable.Map();
};

export const getGroupUsaget = (group) =>
  group.get("cost", false) !== false
    ? "cost"
    : group.get("usage_types", Immutable.Map()).keySeq().get(0, false);

export const getGroupUsageTypes = (group) =>
  group.get("cost", false) !== false
    ? "cost"
    : Immutable.List(
        group.get("usage_types", Immutable.Map()).keySeq().toArray()
      );

export const isGroupMonetaryBased = (group) => getGroupUsaget(group) === "cost";

export const getGroupValue = (group) =>
  isGroupMonetaryBased(group) ? group.get("cost", "") : group.get("value", "");

export const getGroupUsages = (group) =>
  isGroupMonetaryBased(group)
    ? Immutable.List(["cost"])
    : Immutable.List(
        group.get("usage_types", Immutable.Map()).keySeq().toArray()
      );

export const getGroupUnit = (group) =>
  isGroupMonetaryBased(group)
    ? "cost"
    : group
        .get("usage_types", Immutable.Map())
        .valueSeq()
        .get(0, Immutable.Map())
        .get("unit", false);

export const getPlanConvertedIncludes = (
  propertyTypes,
  usageTypes,
  item,
  toBaseUnit = true
  ...
  const convertedIncludes = item
    .get("include", Immutable.Map())
    .withMutations((includesWithMutations) => {
      includesWithMutations
        .get("groups", Immutable.Map())
        .forEach((include, group) => {
          ...
          if (unit && usaget && !isGroupMonetaryBased(include)) {
            ...
            const newConvertedValue =
              newValue === "UNLIMITED" ? newValue : parseFloat(newValue);
            includesWithMutations.setIn(
              ["groups", group, "value"],
              newConvertedValue
            );
          }
...
export const convertServiceBalancePeriodToString = (item) => {
  if (
    ["", "default"].includes(item.getIn(["balance_period", "type"], "default"))
  ) {
    return "default";
  }
  ...
  const balancePeriod =
    unit === "days" ? `tomorrow +${value - 1} days` : `+${value} ${unit}`;
...
export const getAvailableFields = (settings, additionalFields = []) => {
  ...
  const computedFields = settings
    ...
    .map((field) =>
      Immutable.Map({ value: field, label: `${field} (Computed)` })
    )
    .sortBy((field) => field.get("value", ""));
  return Immutable.List([
    ...fields,
    ...computedFields,
    ...Immutable.fromJS(additionalFields),
  ]);
};
...
export const createRateListNameByArgs = (query = Immutable.Map()) =>
  query.reduce((acc, value, key) => `${acc}.${key}.${value}`, "rates");

export const setFieldTitle = (field, entity, keyProperty = "field_name") => {
  if (field.has("title")) {
    return field;
  }
  const entityName = getFieldNameType(
    !entity && field.has("entity") ? field.get("entity") : entity
  );
...
export const addPlayToFieldTitle = (field, plays = Immutable.Map()) => {
  ...
  if (!fieldPlays.isEmpty()) {
    const fieldPlayLabels = fieldPlays
      .reduce(
...
export const inConfigOptionBlackList = (
  config,
  value,
  configName = "exclude"
) => {
...
export const inConfigOptionWhiteList = (
  config,
  value,
  configName = "include"
) => {
...
export const foreignFieldWithoutDates = (foreignField) =>
  foreignField.getIn(["foreign", "translate", "type"], "") !==
  "unixTimeToString";
...
export const getPlayOptions = (availablePlays) =>
  availablePlays
    .map((play) => ({
      value: play.get("name", ""),
...
export const inferPropTypeFromUsageType = (propertyTypes, usageTypes) => {
  const uom = Immutable.List().withMutations((listWithMutations) => {
    if (usageTypes && !usageTypes.isEmpty()) {
      usageTypes.forEach((usaget) =>
        listWithMutations.push(usaget.get("unit", ""))
      );
    }
  });
  const props = Immutable.List().withMutations((listWithMutations) => {
    propertyTypes.forEach((p) => {
      const units = p
        .get("uom", Immutable.List())
        .reduce(
          (acc, curr) => acc.push(curr.get("name", "")),
          Immutable.List()
        );
