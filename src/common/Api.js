...
const checkStatus = (response = null) => {
  if (!response || response.status === API_STATUS_ERROR) {
    // Check status OK
    throw response;
  }
  return response;
...
const buildQueryString = (params = null) => {
  ...
  if (params && Array.isArray(params) && params.length > 0) {
    queryParams = params.reduce((previousValue, currentValue, currentIndex) => {
      ...
      return `${prev}${encodeURIComponent(key)}=${encodeURIComponent(
        currentValue[key]
      )}`;
    }, "?");
  }
...
export const apiBillRunSuccessHandler =
  ...
  (dispatch) => {
    ...
    } else if (status === API_STATUS_WARNING) {
      // Check for warning
      try {
...
export const apiBillRunErrorHandler =
  (error, defaultMessage = "Error, please try again...") =>
  (dispatch) => {
...
const promiseTimeout = (ms, promise, timeOutMessage = "Request timeout") =>
  new Promise((resolve, reject) => {
...
export const openWindowWithPost = (query) => {
  const url = `${getConfig(["env", "serverApiUrl"], "")}${buildApiString(
    query
  )}`;
...
export const buildRequestUrl = (query) =>
  `${getConfig(["env", "serverApiUrl"], "")}${buildApiString(
    query
...
const sendHttpRequest = (query, apiParams = {}) => {
  ...
  const timeout =
    query.timeout || getConfig(["env", "serverApiTimeOut"], 300000);
  console.log("=============");
  console.log(url);
...
export const apiBillRun = (requests, params = {}) => {
  ...
  const promiseRequests = apiRequests.map((request) =>
    sendHttpRequest(request, apiParams)
  );
  // Send All prommisses
  return Promise.all(promiseRequests).then((responses) => {
    const data = responses
      .filter((responce) => responce.status)
      .map(removeStatus);
    ...
    return apiParams.requiredAllSuccess
      ? Promise.reject(mix)
      : Promise.resolve(mix);
  });
};
