...
import { importTypesOptionsSelector } from "@/selectors/importSelectors";
...
class EntityList extends Component {
  static propTypes = {
    ...
    showRevisionBy: PropTypes.oneOfType([PropTypes.bool, PropTypes.string]),
    size: PropTypes.number,
    inProgress: PropTypes.bool,
    forceRefetchItems: PropTypes.oneOfType([PropTypes.bool, PropTypes.number]),
  ...
  componentWillMount() {
    ...
    if (
      (forceRefetchItems || items == null || items.isEmpty()) &&
      fetchOnMount
    ) {
  ...
  componentDidMount() {
    ...
    if (this.isImportEnabled()) {
      ...
      this.props.dispatch(
        getSettings("plugin_actions", { actions: [importName] })
      );
    }
  }
  ...
  componentWillUpdate(nextProps, nextState) {
    ...
    const refreshStringChanged =
      this.props.refreshString !== nextProps.refreshString;
    if (
      pageChanged ||
      sizeChanged ||
      filterChanged ||
  ...
  buildQuery = (props) => {
    ...
    const project = showRevisionBy
      ? { ...projectFields, ...{ to: 1, from: 1, revision_info: 1 } }
      : projectFields;
    ...
    if (showRevisionBy) {
      request.params.push({ states: JSON.stringify(state) });
    }
    return request;
  };
  ...
  parserState = (item) => {
    ...
    return (
      <StateDetails
        item={item}
        itemName={itemType}
        allowManageRevisions={allowManageRevisions}
      />
    );
  };
  ...
  isImportEnabled = () => {
    ...
    return getConfig(["import", "allowed_entities"], Immutable.List()).includes(
      itemType
    );
  };
  ...
  isExportEnabled = () => {
    ...
    return getConfig(["export", "allowed_entities"], Immutable.List()).includes(
      itemType
    );
  };
    ...
    return listActions
      .map((listAction) => {
        const defaultAction = defaultActions.find(
          (action) => action.type === listAction.type
        );
        if (defaultAction) {
  ...
  renderPanelHeader = () => {
    ...
    const itemsTypeName = getConfig(
      ["systemItems", entityKey, "itemsName"],
      noCase(itemsType)
    );
    return (
      <div>
  ...
  getActions = () => {
    ...
    return actions.map((action) => {
      switch (action.type) {
        case "edit":
          return Object.assign(editAction, action);
        case "view":
          return Object.assign(viewAction, action);
        default:
          return action;
      }
    });
  ...
  renderList = () => {
    ...
    const fields = !showRevisionBy
      ? tableFields
      : this.addStateColumn(tableFields);
    return (
      <List
        sort={sort}
        items={items}
...
const mapStateToProps = (state, props) => {
  ...
  return {
    ...
    typeSelectOptions: importTypesOptionsSelector(
      state,
      props,
      "importer",
      itemType
