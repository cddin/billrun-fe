...
class EditMenuItem extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    return (
      !Immutable.is(this.props.item, nextProps.item) || this.state !== nextState
    );
  }
  ...
  onChangeRoles = (roles) => {
    ...
    this.setState({
      menuItem: menuItem.set("roles", Immutable.List(rolesList)),
    });
  };
  ...
  renderEditModal = () => {
    ...
    return (
      <ModalWrapper
        show={editMode}
        onOk={this.onSaveAdvencedEdit}
        onCancel={this.onCancelAdvencedEdit}
        title={`Edit ${currentTitle} Details`}
      >
        <Form horizontal>
          <FormGroup>
  ...
  render() {
    ...
    return (
      ...
      >
        <Col
          md={6}
          style={{ color: "#625EE5", cursor: "pointer" }}
        ...
        <Col md={2} className="text-right" style={checkboxStyle}>
          <Field
            onChange={this.onChangeShowHide}
            value={show}
            fieldType="checkbox"
            disabled={!editShow}
          />
