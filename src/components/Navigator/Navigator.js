...
class Navigator extends Component {
      ...
      .forEach((item) => {
        if (
          item
            ...
            .filter((subMenu) => router.isActive(subMenu.get("route", "")))
            .size > 0
        ) {
  ...
  renderSubMenu = (item, key) => {
    ...
    const activeSubMenus = subMenus.filter((subMenu) =>
      router.isActive(subMenu.get("route", ""))
    );
  ...
  renderMenu = (menuItem, key) =>
    menuItem.get("subMenus")
      ? this.renderSubMenu(menuItem, key)
      : this.renderMenuItem(menuItem, key);
  ...
  renderMenuItem = (item, key) => {
    ...
    return (
      <OverlayTrigger
        key={`${id}_${key}`}
        placement={collapseSideBar ? "right" : "top"}
        overlay={tooltip}
      >
        {link}
      </OverlayTrigger>
  ...
  render() {
    ...
    const { userName, companyNeme, menuItems, logo, collapseSideBar, router } =
      this.props;
