...
export const saveSettings =
  ...
  (dispatch, getState) => {
    ...
    let categoriesToSave = Array.isArray(categories)
      ? categories
      : [categories];
    categoriesToSave = categoriesToSave.filter(
      (category) => !ignoreCategories.includes(category)
    );
    if (categoriesToSave.length === 0) {
      const warning = {
        data: [{ data: { status: 2, warnings: "No settings to save" } }],
      };
    ...
    return apiBillRun(queries)
      .then((success) =>
        dispatch(apiBillRunSuccessHandler(success, successMessage))
      )
...
export const getSettings =
  (categories = [], data = {}) =>
  (dispatch) => {
    const categoriesToSave = Array.isArray(categories)
      ? categories
      : [categories];
    const queries = categoriesToSave.map((category) =>
      getSettingsQuery(category, data)
    );
    return apiBillRun(queries)
      .then((success) => {
...
export const getCurrencies = () => (dispatch) => {
  ...
  if (
    cache &&
    moment(cache.time).add(cacheForMinutes, "minutes").isAfter(now)
  ) {
  ...
  return apiBillRun(query)
    ...
    .catch((error) =>
      dispatch(apiBillRunErrorHandler(error, "Error retrieving currencies"))
    );
};
...
export const saveSharedSecret = (secret, mode) => (dispatch) => {
  ...
  const query =
    mode === "remove"
      ? disableSharedSecretQuery(secret)
      : saveSharedSecretQuery(secret);
  return apiBillRun(query)
    .then((success) => {
      ...
      return dispatch(
        apiBillRunSuccessHandler(success, `The secret key was ${action}`)
      );
    })
    .catch((error) =>
      dispatch(apiBillRunErrorHandler(error, "Error saving secret"))
    );
};
